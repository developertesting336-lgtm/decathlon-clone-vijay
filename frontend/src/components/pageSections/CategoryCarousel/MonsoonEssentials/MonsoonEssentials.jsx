...
const MonsoonEssentials = ({
  ...
  const fetchSectionData = useCallback(async () => {
    try {
      setLoading(true);
      const res = await api.get("/pages/slug/monsoon-essentials");
      ...
      console.error("MonsoonEssentials carousel error:", err);
      setCategories([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <section className="category-carousel-monsoon-essentials">
      {sectionTitle && sectionTitle !== "rain" && (
        <h2 className="monsoon-category-title">{sectionTitle}</h2>
      )}
      <div className="monsoon-category-track">
        {categories.map((cat, idx) => (
          <div
            key={cat._id || idx}
            className="monsoon-category-card"
            onClick={() => handleCardClick(cat)}
          >
            <div className="monsoon-category-circle">
              {cat.image ? (
                <img
                ...
                <div className="monsoon-category-placeholder">
                  {cat.name}
                </div>
              )}
            </div>
