...
const BagsBackpacks = ({
  ...
  const parseItem = (c, i) => {
    ...
    let catName = (c.title || c.name || catObj.name || "Category").trim();
    if (catName.toLowerCase() === "backpacks") {
      catName = "Duffle Bags";
    }
  ...
  const fetchSectionData = useCallback(async () => {
    try {
      setLoading(true);
      const res = await api.get("/pages/slug/bags-backpacks");
      ...
      console.error("BagsBackpacks carousel error:", err);
      setCategories([]);
    } finally {
      setLoading(false);
    }
    ...
    <section className="category-carousel-bags-backpacks">
      ...
      <div className="bags-category-track">
        {categories.map((cat, idx) => (
          ...
          >
            <div className="bags-category-image-box">
              {cat.image ? (
                <img
                ...
                <div className="bags-category-placeholder">
                  {cat.name}
                </div>
              )}
            </div>
