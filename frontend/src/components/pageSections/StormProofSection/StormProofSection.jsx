...
const StormProofSection = ({ section, data, style, customProducts, title, subtitle }) => {
  ...
  const displaySubtitle = subtitle || sectionData.subtitle || "Storm Proof.";
  const displayTitle = title || sectionData.title || "Style\nApproved.";
  ...
  const fetchSection = useCallback(async () => {
    try {
      ...
      const found = pageSections.find(
        (item) =>
          item.type === "storm-proof" ||
          item.name === "Storm Proof Section" ||
          (item.name && item.name.toLowerCase().includes("storm"))
      );
